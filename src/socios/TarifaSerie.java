...
/**
 * Esta clase representa el tipo de tarifa sencilla Serie. 
 * 
 * @author Jaime Monedero y Elena Lechuga
 */
public class TarifaSerie extends Tarifa {
	 ...
	 */
	public TarifaSerie(Socio socio, boolean plus) {
		super(socio, plus);
	}
	 ...
	 * @param plus Define si la tarifa es plus (dia(s) extra de alquiler) o no
	 */
	public TarifaSerie(int idTarifa, int socio, Calendar fechaInicio, Calendar fechaFin, boolean plus) {
		super(idTarifa, socio, fechaInicio, fechaFin, plus);
	}
	
	@Override
	public String toString() {
		return "TS,"+ getId() +","+ getSocio().getNumSocio() + "," + getFechaInicio().get(Calendar.DAY_OF_MONTH)+"."+(getFechaInicio().get(Calendar.MONTH)+1)+"."+getFechaInicio().get(Calendar.YEAR)+ "," 
	...
	public boolean cubre(String tipo) {
		if (tipo.equals("Pelicula")) return false; 
		else if (tipo.equals("Musica")) return false; 
		else if (tipo.equals("Serie")) return true; 
		else return false;
	}
